...
def read_multiple_inputs(train_paths, test_paths):
    ...
    for train_path in train_paths:
        #  Disable warnings when read large number of row csv : set low_memory=False
        #  # https://stackoverflow.com/questions/24251219/pandas-read-csv-low-memory-and-dtype-options
        train_i = pd.read_csv(train_path, low_memory=False)
    ...
    for test_path in test_paths:
        #  Disable warnings when read large number of row csv : set low_memory=False
        #  # https://stackoverflow.com/questions/24251219/pandas-read-csv-low-memory-and-dtype-options
        test_i = pd.read_csv(test_path, low_memory=False)
...
def label_encoding_loaded(all_data):
    ori_encoder_dict = np.load('price_estimate/output/saved_models/label_encoder.npy', allow_pickle=True)
...
def box_cox_transform_skewed_features_loaded(record):
    from scipy.special import boxcox1p
    # LOAD
    skewed_features = np.load('price_estimate/output/saved_models/skewed_features.npy', allow_pickle=True)
    lam = 0.15
    # print("[check-before]:", all_data["LotArea"])
    for feat in skewed_features:
        # all_data[feat] += 1
...
def convert_crawled_to_input_data(crawled_paths, out_train_file_path, out_test_file_path):
    ...
    for rec in range(crawled_datas.shape[0]):
        ...
        if is_valid_record(width[rec], height[rec], area[rec], price[rec], form_re[rec], province[rec]):
            # if id_rec < crawled_datas.shape[0] / 1.5:
            if id_rec < 20000:
...
def is_valid_record(width, height, area, price, form_re, province):
    i = 0
    import math
    ...
    if math.isnan(price) or price < 0:
        i += 1
    if math.isnan(form_re) or form_re < 0:
        i += 1
    if math.isnan(province) or province < 0:
        i += 1

    if i >= 1:
        return False
    else:
        if (abs(width * height - area))/area > 0.1 or price / area < 1000000 or price / area > 500000000:
            return False
        else:
            return True
